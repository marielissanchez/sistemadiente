<?php
	...
	function usuarioExiste($usuario)
	{
		...
		$stmt = $mysqli->prepare("SELECT id FROM usuarios WHERE usuario = $usuario LIMIT 1");
		$stmt->execute();
		$stmt->store_result();
		$num = $stmt->num_rows;
		$stmt->close();
	...
	{
		$mysql=new mysqli("localhost","root","","enfermeria");
		
		$stmt = mysqli_query($mysql,"SELECT * FROM usuario WHERE correo = '$email'");
		$rows =0;
		if($n=mysqli_fetch_array($stmt)){
		$rows =1;
		}
		if($rows > 0) {
			return true;
	...
	function registraUsuario($usuario, $pass_hash, $nombre, $email){
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		use mail\src\PHPMailer;
		use mail\src\Exception;
	...
	function login($usuario, $password)
	{
		...
		$stmt = mysqli_query($mysql,"SELECT * FROM usuario WHERE usuario = '$usuario'");
		$rows =0;
		if($n=mysqli_fetch_array($stmt)){
		$rows =1;
		}
		if($rows > 0) {
			...
			if(true==true){
				...
				$validaPassw = password_verify($password, $n['contraseña']);
	...
	function generaTokenPass($user_id)
	{
		...
		$stmt = $mysqli->prepare("UPDATE usuario SET token_password=?, password_request=1 WHERE id = ?") or die ($mysqli->error);
		$stmt->bind_param('ss', $token, $user_id) or die ($mysqli->error);
		$stmt->execute();
		$stmt->close();
	...
	{
		$mysql=new mysqli("localhost","root","","enfermeria");
		$stmt = mysqli_query($mysql,"SELECT $campo FROM usuario WHERE $campoWhere = '$valor' LIMIT 1") or die(mysqli_error($mysql));
		$rows =0;
		if($n=mysqli_fetch_array($stmt)){
			return $n[$campo];
		}
